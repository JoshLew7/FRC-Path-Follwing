package frc.team3310.robot.auto.actions;
...
public class CollectAccelerationData implements Action {
     ...
     */
    public CollectAccelerationData(List<DriveCharacterization.AccelerationDataPoint> data, boolean reverse,
            boolean turn) {
        mAccelerationData = data;
        // mHighGear = highGear;
        mReverse = reverse;
        mTurn = turn;
        mCSVWriter = new ReflectingCSVWriter<>("/home/lvuser/ACCEL_DATA.csv",
    ...
    public void start() {
        ...
        mDrive.setOpenLoop(new DriveSignal((mReverse ? -1.0 : 1.0) * kPower,
    ...
    @Override
    public void update() {
        double currentVelocity = (Math.abs(mDrive.getLeftVelocityNativeUnits())
                + Math.abs(mDrive.getRightVelocityNativeUnits())) / 4096.0 * Math.PI * 10;
        ...
        mAccelerationData.add(new DriveCharacterization.AccelerationDataPoint(currentVelocity, // convert to radians per
                                                                                               // second
